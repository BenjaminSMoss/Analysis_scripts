...
function get_upper_bound(y_index_,switching_values,y_array,pureZ_array)
    ...
    upper_bound = fill(0, size(pureZ_array,2))
    ...
    println("potential is $current_y")
...
function get_lower_bound(y_index__,switching__values,y__array,pureZ_array_,LB_array)
    current_y=y__array[y_index__]
    lower_bound = fill(0, size(pureZ_array_,2))
    for i=1:size(switching__values)[1]
        if current_y<=switching__values[1]
            lower_bound[1] = 0
            break
        elseif current_y<=switching__values[i]&&current_y>=switching__values[i-1]
            lower_bound[1:i].=LB_array[1:i]
            
        elseif current_y>=switching__values[end]
            lower_bound=LB_array
        
        end 
    end 
    println("lower bound array set to $lower_bound")
